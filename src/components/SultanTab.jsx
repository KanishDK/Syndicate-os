...
const SultanTab = ({ state, handleChoice, buyHype, buyBribe }) => {
    ...
    return (
        <div className="max-w-6xl mx-auto h-full flex flex-col gap-6 p-1">

            {/* HEADER SECTION */}
            <div className="flex flex-col md:flex-row gap-4 items-center justify-between border-b border-theme-border-subtle pb-4">
                <div>
                    <h2 className="text-3xl font-black uppercase tracking-tighter text-theme-warning flex items-center gap-3 font-terminal">
                        <i className="fa-solid fa-comment-dots"></i> {t('sultan.title')}
                    </h2>
                    <p className="text-xs text-theme-text-secondary mt-1 font-mono uppercase tracking-widest opacity-60">
                        {t('sultan.subtitle')}
                    </p>
                </div>

                {/* DYNAMIC GREETING */}
                <GlassCard className="w-full md:w-auto p-4 flex gap-4 items-center min-w-[300px]" variant="base">
                    <div className="w-10 h-10 rounded-full bg-theme-warning/20 border border-theme-warning/30 flex items-center justify-center shrink-0">
                        <i className="fa-solid fa-store text-lg text-theme-warning"></i>
                    </div>
                    <div>
                        <p className="text-xs font-bold text-theme-warning uppercase tracking-widest mb-1">{CONFIG.pols.name}</p>
                        <p className="text-[10px] text-theme-text-primary italic leading-relaxed">
                            "{(() => {
            ...
            <div className="p-6 flex-1 flex flex-col">
                <div className="flex justify-between items-start mb-4">
                    <div>
                        ...
                        <h3 className="text-xl font-black text-theme-text-primary uppercase tracking-tighter">{getMissionTitle()}</h3>
                        <p className="text-xs text-theme-text-secondary leading-relaxed mt-1" dangerouslySetInnerHTML={{ __html: getMissionText() }} />
                    </div>
                    <div className={`w-10 h-10 rounded-full border border-white/10 bg-white/5 flex items-center justify-center ${mission.isDaily ? 'text-theme-info' : 'text-theme-warning'}`}>
